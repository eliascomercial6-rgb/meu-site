...
// Focus mark loaded from the public folder
const FOCUS_MARK_NEW = '/focus-mark.png';
...
export default function AdminSidebar({ activeTab, onTabChange, onLogout, userName, avatarUrl }: AdminSidebarProps) {
  ...
  return (
    <>
      ...
      <aside className="hidden md:flex w-[248px] shrink-0 md:h-full flex-col justify-between border-r border-white/5 bg-[#0a0a0c] relative overflow-hidden">
        ...
        <div className="relative flex flex-col min-h-0">
          {/* Brand plate — the mark stands alone now, no bordered black
              square container behind it. */}
          <div className="px-6 pt-7 pb-6 flex items-center relative">
            <div className="min-w-0">
              <img src="/focus-logo-metallic.png" alt="FocusPortfolio" className="h-4 w-auto object-contain" loading="eager" />
              <p className="text-[9px] text-neutral-600 font-sans uppercase tracking-widest mt-1">Painel Autoral</p>
            </div>
          </div>

          {/* Index rail — icon + label, no frame numbers */}
            ...
            {NAV_ITEMS.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group relative w-full flex items-center gap-3 pl-3 pr-3 py-2.5 rounded-xl text-sm font-medium tracking-wide transition-all duration-300 ease-out ${
                    isActive
        ...
        {/* User frame — real avatar photo when available, initials otherwise */}
          ...
          {userName && (
            <div className="flex items-center gap-3 px-2.5 py-2 mb-2 rounded-xl">
              {avatarUrl ? (
                ...
                <div className="relative w-8 h-8 rounded-full bg-black border border-white/10 shrink-0 flex items-center justify-center font-sans text-[10px] text-[var(--app-accent)] font-bold tracking-wide">
                  {initials}
      ...
      <div className="md:hidden sticky top-0 z-[35] border-b border-white/5 bg-[#0a0a0c]/95 backdrop-blur-xl">
